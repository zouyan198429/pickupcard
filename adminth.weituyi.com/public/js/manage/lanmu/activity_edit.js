...
function ajax_form(){
    ...
    var code_len = $('input[name=code_len]').val();
    if(!judge_validate(4,'兑换码长度',code_len,true,'digit','','')){
        return false;
    }
    if(!judge_validate(4,'兑换码长度',code_len,true,'range',2,20)){
        return false;
    }

    var default_open_status = $('input[name=default_open_status]:checked').val() || '';
    var judge_seled = judge_validate(1,'生成兑换码时默认状态',default_open_status,true,'custom',/^[12]$/,"");
    if(judge_seled != ''){
        layer_alert("请选择生成兑换码时默认状态",3,0);
        //err_alert('<font color="#000000">' + judge_seled + '</font>');
        return false;
    }
    ...
    var activity_theme = $('input[name=activity_theme]').val();
    if(!judge_validate(4,'供货商',activity_theme,false,'length',1,50)){
        return false;
    }

    var activity_subtitle = $('input[name=activity_subtitle]').val();
    if(!judge_validate(4,'商家广告语',activity_subtitle,false,'length',1,100)){
        return false;
    }

    var tag_price = $('input[name=tag_price]').val();
    if(!judge_validate(4,'吊牌价',tag_price,true,'doublepositive','','')){
        return false;
    }

    var price = $('input[name=price]').val();
    if(!judge_validate(4,'商品价',price,true,'doublepositive','','')){
        return false;
    }

    var freight_price = $('input[name=freight_price]').val();
    if(!judge_validate(4,'快递费',freight_price,true,'doublepositive','','')){
        return false;
    }

    var insured_price = $('input[name=insured_price]').val();
    if(!judge_validate(4,'保价费',insured_price,true,'doublepositive','','')){
        return false;
    }
