...
<script type="text/javascript">
    var SAVE_URL = "{{ url('api/seller/activity/ajax_save') }}";// ajax保存记录地址
    var LIST_URL = "{{url('seller/activity')}}";//保存成功后跳转到的地址

    var BEGIN_DATE = "{{ $info['begin_time'] or '' }}" ;//开始日期
    var END_DATE = "{{ $info['end_time'] or '' }}" ;//结束日期

    // 上传图片变量
    var FILE_UPLOAD_URL = "{{ url('api/seller/upload') }}";// 文件上传提交地址 'your/file/upload/url'
    var PIC_DEL_URL = "{{ url('api/seller/upload/ajax_del') }}";// 删除图片url
...
<script src="{{ asset('/js/seller/lanmu/activity_edit.js') }}?2"  type="text/javascript"></script>
