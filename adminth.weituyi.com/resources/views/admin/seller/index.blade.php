...
<!--           <span class="lbl">全选</span>
 -->        </label>
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/admin/seller/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('admin/seller/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('admin/seller/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "商家" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('admin/seller/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('admin/seller/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/admin/seller/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/admin/seller/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('admin/seller/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('admin/seller/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/admin/seller/import') }}";//导入EXCEL地址
