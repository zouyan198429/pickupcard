...
--}}
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/company/city/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('company/city/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('company/city/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "城市" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('company/city/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('company/city/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/company/city/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/company/city/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('company/city/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('company/city/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/company/city/import') }}";//导入EXCEL地址
      ...
      var NOTICE_LIST_URL = "{{ url('company/notice') }}"; //城市公告管理
      var FEESCALE_MODIFY_URL = "{{ url('company/feeScale/add/') }}/"; //收费标准管理[文字]
      var FEESCALETIME_MODIFY_URL = "{{ url('company/feeScaleTime/addBath/') }}/"; //收费标准管理[时间段]
  ...
  <script src="{{ asset('js/company/lanmu/city.js') }}"  type="text/javascript"></script>
</body>
</html>
