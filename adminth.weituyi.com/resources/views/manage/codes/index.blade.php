...
<div class="mm">
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/manage/codes/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('manage/codes/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('manage/codes/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "兑换码" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('manage/codes/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('manage/codes/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/manage/codes/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/manage/codes/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('manage/codes/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('manage/codes/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/manage/codes/import') }}";//导入EXCEL地址
      var IMPORT_EXCEL_CLASS = "import_file";// 导入EXCEL的file的class

      var OPEN_ALL_URL = "{{ url('api/manage/codes/ajax_open_all') }}";//开启所有[根据活动id]
      var OPEN_URL = "{{ url('api/manage/codes/ajax_open') }}";//单个或批量开启地址
      var CLOSE_ALL_URL = "{{ url('api/manage/codes/ajax_close_all') }}";//关闭所有[根据活动id]
      var CLOSE_URL = "{{ url('api/manage/codes/ajax_close') }}";//单个或批量关闭地址
      var CURRENT_ACTIVITY_ID = "{{ $activity_id or 0 }}";// 当前操作的活动id
  </script>
  <script src="{{asset('js/common/list.js')}}"></script>
  <script src="{{ asset('js/manage/lanmu/codes.js') }}"  type="text/javascript"></script>
</body>
</html>
