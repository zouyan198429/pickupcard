...
--}}
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/manage/city/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('manage/city/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('manage/city/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "城市" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('manage/city/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('manage/city/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/manage/city/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/manage/city/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('manage/city/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('manage/city/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/manage/city/import') }}";//导入EXCEL地址
      var IMPORT_EXCEL_CLASS = "import_file";// 导入EXCEL的file的class

      var PROVINCE_CHILD_URL  = "{{url('api/manage/city/ajax_get_child')}}";// 获得地区子区域信息
      var CITY_CHILD_URL  = "{{url('api/manage/city/ajax_get_child')}}";// 获得地区子区域信息

      const PROVINCE_ID = "{{ $info['province_id'] or 0}}";// 省默认值
      const CITY_ID = "{{ $info['city_id'] or 0 }}";// 市默认值
      const AREA_ID = "{{ $info['area_id'] or 0 }}";// 区默认值

      var NOTICE_LIST_URL = "{{ url('manage/notice') }}"; //城市公告管理
      var FEESCALE_MODIFY_URL = "{{ url('manage/feeScale/add/') }}/"; //收费标准管理[文字]
      var FEESCALETIME_MODIFY_URL = "{{ url('manage/feeScaleTime/addBath/') }}/"; //收费标准管理[时间段]
  ...
  <script src="{{ asset('js/manage/lanmu/city.js') }}"  type="text/javascript"></script>
</body>
</html>
